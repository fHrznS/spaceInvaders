...
namespace SpaceInvaders.Entities.Bosses {
    internal class Saigai : BasicBoss {
        int bulletTypeDeadly = 7;
        int bulletTypeHealing = 8;
        bool firstMove = true;
        ...
        internal override void Update(Vector2 playerPos) {
            ...
            if (attackTimer <= Time.ToFrames(seconds: 55, minutes: 4) && attackTimer >= Time.ToFrames(seconds: 40, minutes: 4) && attackTimer % 40 == 0) {
                MainGame.newEnemyBullet<Bullet>(center, new(1.2f * direction.X, 1.4f), bulletTypeDeadly, multID, healBoss: true, bossBullet: true);
                MainGame.newEnemyBullet<Bullet>(center, new(0.35f * direction.X, 1.4f), bulletTypeDeadly, multID, healBoss: true, bossBullet: true);

                MainGame.newEnemyBullet<Bullet>(center, new(1.2f * -direction.X, 1.4f), bulletTypeDeadly, multID, healBoss: true, bossBullet: true);
                MainGame.newEnemyBullet<Bullet>(center, new(0.35f * -direction.X, 1.4f), bulletTypeDeadly, multID, healBoss: true, bossBullet: true);
            }
            if (attackTimer <= Time.ToFrames(seconds: 45, minutes: 4) && attackTimer >= Time.ToFrames(seconds: 35, minutes: 4) && attackTimer % 30 == 0) {
                MainGame.newEnemyBullet<Bullet>(center, new(rng.Next(-2,3) * (float)rng.NextDouble() , 2), bulletTypeDeadly, multID, healBoss: true, bossBullet: true);
            }
            ...
            if (attackTimer <= Time.ToFrames(seconds: 30, minutes: 4) && attackTimer >= Time.ToFrames(seconds: 10, minutes: 4) && attackTimer % 40 == 0 && attackTimer % (40 * 6) != 0) {
                ...
                for (int i = 0; i < 9; i++) {
                    ...
                    MainGame.newEnemyBullet<Bullet>(new(8 + 16 * i, -8), new(0, 1.75f), bulletTypeDeadly, multID, bossBullet: true, healBoss: true, damage: 2);
                }
            ...
            if (attackTimer <= Time.ToFrames(seconds: 30, minutes: 2) && attackTimer >= Time.ToFrames(seconds: 0, minutes: 2) && attackTimer % 80 == 0) {
                ...
                for (int i = 0; i < 9; i++) {
                    ...
                    MainGame.newEnemyBullet<Bullet>(new(8 + 16 * i, -8), new(0, speed), bulletTypeHealing, multID, bossBullet: true, healBoss: true, damage: 0);
                }
            }
            ...
            if (attackTimer <= Time.ToFrames(seconds: 50, minutes: 1) && attackTimer >= Time.ToFrames(seconds: 50, minutes: 0) && attackTimer % 90 == 0) {
                ...
                MainGame.newEnemyBullet<Bullet>(new Vector2(0 + (16 * bulletOffset), -8), new(0, 3f), bulletTypeHealing, multID, bossBullet: true, damage: 0);
                MainGame.newEnemyBullet<Bullet>(new Vector2(32 + (16 * bulletOffset), -8), new(0, 3f), bulletTypeHealing, multID, bossBullet: true, damage: 0);
                MainGame.newEnemyBullet<Bullet>(new Vector2(64 + (16 * bulletOffset), -8), new(0, 3f), bulletTypeHealing, multID, bossBullet: true, damage: 0);
                MainGame.newEnemyBullet<Bullet>(new Vector2(96 + (16 * bulletOffset), -8), new(0, 3f), bulletTypeHealing, multID, bossBullet: true, damage: 0);
                MainGame.newEnemyBullet<Bullet>(new Vector2(128 + (16 * bulletOffset), -8), new(0, 3f), bulletTypeHealing, multID, bossBullet: true, damage: 0);
            ...
            if (attackTimer <= Time.ToFrames(40, minutes: 1) && attackTimer >= Time.ToFrames(50) && attackTimer % 45 == 0) {
                MainGame.newEnemyBullet<BulletHoming>(center, new Vector2(0, 1.4f), bulletTypeDeadly, multID, bossBullet: true, healBoss: true);
            }

            // Downwards bullet
            if (attackTimer <= Time.ToFrames(20, minutes: 1) && attackTimer >= Time.ToFrames(50) && attackTimer % 60 == 0) {
                MainGame.newEnemyBullet<Bullet>(center, new Vector2(0, 2f), bulletTypeDeadly, multID, bossBullet: true, healBoss: true);
            }
